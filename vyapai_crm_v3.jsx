...
function MetricCard({label,labelHi,value,sub,color,icon}){
  return <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"12px 14px",minWidth:0}}>
...
// ─── DASHBOARD ───────────────────────────────────────────
function Dashboard({customers,invoices,account}){
  ...
  return <div style={{paddingBottom:20}}>
    ...
    <div style={{background:`linear-gradient(145deg,#0C1228 0%,#1B2BB8 45%,#4B6FFF 100%)`,borderRadius:20,padding:"20px",marginBottom:16,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",gap:10,marginTop:14}}>
        ...
        <div style={{flex:1,background:"rgba(255,255,255,0.12)",borderRadius:10,padding:"9px",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <span style={{fontSize:16}}>📊</span>
          <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
      <MetricCard label="Customers" labelHi="ग्राहक" value={activeC} sub="Active" color={C.pM} icon="👥"/>
      <MetricCard label="Unpaid" labelHi="बकाया" value={invoices.filter(i=>i.status==="Unpaid").length} sub="Invoices" color={C.r1} icon="🧾"/>
      <MetricCard label="Total Sales" labelHi="कुल बिक्री" value={INR(totalSales)} sub="This month" color={C.g1} icon="📈"/>
      <MetricCard label="Partial" labelHi="आंशिक" value={invoices.filter(i=>i.status==="Partial").length} sub="Invoices" color={C.o1} icon="⏳"/>
    </div>
...
export default function App(){
  ...
  const [account,setAccount]=useState({
    name:"Rajesh Kumar",business:"Kumar Traders",phone:"9876543210",
    city:"Lucknow",state:"Uttar Pradesh",category:"Kirana / Grocery",
    gstin:"09AABCU9603R1ZX",email:"rajesh@kumartraders.com",
    about:"15 saal purani kirana dukaan. Main products: grocery, dal, chawal, tel, masale. 200+ regular customers."
  });

  const addCustomer=form=>{
    setCustomers(c=>[...c,{...form,customer_id:`C00${c.length+1}`,balance:0}]);
  };
  const addInvoice=form=>{
    const cust=customers.find(c=>c.customer_id===form.customer_id);
    if(!cust)return;
    setInvoices(i=>[{invoice_id:`INV-00${i.length+1}`,customer_id:form.customer_id,customer_name:cust.business_name,date:"2026-03-20",total_amount:Number(form.total_amount)||0,paid:0,status:"Unpaid",notes:form.notes},...i]);
  };
  ...
  return <>
    ...
    <div style={{maxWidth:430,margin:"0 auto",background:C.bg,minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:"Poppins,sans-serif",color:C.txt}}>
      ...
      <div style={{flex:1,overflowY:tab==="chat"?"hidden":"auto",padding:tab==="chat"?0:"14px 12px",display:"flex",flexDirection:"column"}}>
        {tab==="dashboard"&&<Dashboard customers={customers} invoices={invoices} account={account}/>}
        ...
        {tab==="account"&&<Account account={account} setAccount={setAccount}/>}
      </div>
